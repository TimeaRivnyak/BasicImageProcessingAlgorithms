function mrf = gmrf_defineRegions(params, mrf)
    ...
    for ind=1:params.NumOfClasses
        ...
        cropped = im2double(params.InputImage(rects{ind}.y1 : rects{ind}.y2, rects{ind}.x1 : rects{ind}.x2));
        mrf.gauss_params{ind}.mean = mean(cropped(:));
        mrf.gauss_params{ind}.dev = std(cropped(:));
        mrf.logProbs{ind} = -log(normpdf(im2double(params.InputImage), mrf.gauss_params{ind}.mean, mrf.gauss_params{ind}.dev));
